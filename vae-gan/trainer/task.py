...
class Evaluator(object):
  ...
  def evaluate(self, num_eval_batches=None):
    ...
    with tf.Graph().as_default() as graph:
      self.tensors = self.model.build_eval_graph(self.data_dir,
            ...
            try:
              ...
              for _ in range(4):
                _, self.global_step = session.run(
                    [self.tensors.encoder_optimizer, self.tensors.global_step])

              for _ in range(2):
                _, gen_result, self.global_step = session.run([
                    self.tensors.generator_optimizer,
                    self.tensors.cost_generator, self.tensors.global_step
                ])
                if gen_result < LOWER_THRESHOLD:
                  break

              disc_result, self.global_step = session.run([
                  self.tensors.cost_discriminator,
                  self.tensors.global_step,
              ])

              if disc_result > LOWER_THRESHOLD and gen_result < UPPER_THRESHOLD:
                _, self.global_step = session.run([
                    self.tensors.discriminator_optimizer,
                    self.tensors.global_step
                ])
